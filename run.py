...
def add_category():
    if request.method == 'POST':
        if not request.form['title'] in CATEGORIES:
    ...
    return render_template('add_category.html')  # this is a get request
...
def view_recipes(category_title):
    ...
    # am  passing the recipes dict to the template
...
def add_recipe():
    if request.method == 'POST':
        if not request.form['title'] in recipes:
            recipes[request.form['title']] = Recipe(request.form['title'],request.form['description'])
            flash('recipe category added successfully', 'success')
            return render_template('recipes.html', recipes=recipes)
        flash('recipe already exists', 'danger')
...
def recipe_details(title):
    recipe = recipes[title]
    return render_template("details.html", title=recipe.title, description=recipe.description)
